...
public class PushRequestHandler implements RequestHandler, SessionExpiredHandler {
	...
	public PushRequestHandler(VaadinServletService service) throws ServiceException {
		...
		pushHandler.setLongPollingSuspendTimeout(atmosphere.getAtmosphereConfig()
				.getInitParameter(Constants.SERVLET_PARAMETER_PUSH_SUSPEND_TIMEOUT_LONGPOLLING, -1));
	...
	/**
	 * Returns an AtmosphereFramework instance which was initialized in the servlet
	 * context init phase by {@link JSR356WebsocketInitializer}, if such exists.
	 */
	...
	static AtmosphereFramework initAtmosphere(final ServletConfig vaadinServletConfig) {
		...
		atmosphere.addInitParameter(ApplicationConfig.DISABLE_ATMOSPHEREINTERCEPTORS,
		...
		// comment out the next line to activate the JSR356 detection again
		atmosphere.addInitParameter(ApplicationConfig.WEBSOCKET_SUPPRESS_JSR356, "true");
		// or add JSR356AsyncSupport
		// Enable JSR356AsyncSupport with Websocket
		// ApplicationConfig.PROPERTY_COMET_SUPPORT = org.atmosphere.cpr.asyncSupport
		// atmosphere.addInitParameter(ApplicationConfig.PROPERTY_COMET_SUPPORT,
		...
		try {
			...
			atmosphere.setAsyncSupport(new Jetty9AsyncSupportWithWebSocket(atmosphere.getAtmosphereConfig()));
